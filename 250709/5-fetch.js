// XMHttpRequest VS Fetch API
/* 
- Fetch는 객체 생성 불필요 (함수라서 인자로 전달 가능)
- Promise 기반 (then, catch)
- 간결하고 가독성 좋음
- 최신 표준 방식
*/

// 1. Promise (객체)
// 비동기 통신 결과를 즉싱 알 수 없지만,
// 결과가 준비되면, 이행하거나 거절될 것이라고 약속한다.
...
orderCoffee
    // resolve 작업 성공, 다음 일 처리
    .then((message) => {
        console.log(message);
    })
    // reject 문제 발생, 어떻게 처리하지?
    .catch((message) => {
        console.log(message);
    })
    // 결과가 resolve던 reject던 마지막에 무조건 실행되는 것(마무리)
    .finally(() => {
        console.log("======커피 완료======");
    });
